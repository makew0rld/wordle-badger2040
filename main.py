...
print(WORD)
    ...
    #display.pen(0)
    #display.font(HEADER_FONT)
    #display.text("Wordle", HALF_W+20, HALF_H-51, rotation=90)
    #display.text("by makeworld", HALF_W-20, HALF_H-29, scale=0.5, rotation=90)
    #display.update()
    #sleep(3)
...
display.pen(0)
display.font(HEADER_FONT)
display.text("Wordle", WIDTH-30, HALF_H-51, rotation=90)
draw_grid()
display.update()
...
display.update_speed(UPDATE_TURBO)
...
while True:
    if display.pressed(BUTTON_B):
        ...
        # Update position
        pos_x = max(0, pos_x-1)
        display.update_speed(UPDATE_FAST) # Better update
    ...
    display.update()
    display.update_speed(UPDATE_TURBO) # Reset update speed in case it was changed
...
if "".join(grid[pos_y]) == WORD:
    # Won
    display.text("You won!", 30, HALF_H-55, rotation=90, scale=0.8)
else:
    # Lost
    display.text(WORD, 30, int(HALF_H-display.measure_text(WORD)/2), rotation=90)
